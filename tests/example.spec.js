...
const { getLatestOtp } = require("../utils/gmailHelper");
...
test.only("Browser context playwright test", async ({ browser }) => {
  ...
  await page.goto(
    "https://sso.teachable.com/secure/9521/identity/sign_up/otp?wizard_id=BEi7-P6ZvMXI1VOuPwFbEbrUvqeWbny-i7noy1y2ulZkZx3YM0UJs9RdXXRANTmTPoRj01DiK90cHelIe6hbpw",
  );
  ...
  // Click button — OTP field appears on same page
  await page.locator("#otp-login-btn").click();

  console.log("✅ Button clicked, waiting for OTP field...");

  // Wait for OTP field to appear on same page
  await page.waitForSelector("#otp", { state: "visible", timeout: 15000 });

  console.log("✅ OTP field visible, fetching from Gmail...");

  // Fetch OTP from Gmail
  const otp = await getLatestOtp(
    "from:no-reply@teachable.com subject:sign up code",
    /\b\d{6}\b/,
    5,
    6000,
  );
  ...
  // Add this AFTER filling OTP to find the right button
  const buttons = await page.$$eval('button, input[type="submit"]', (els) =>
    els.map((el) => ({ id: el.id, text: el.innerText, type: el.type })),
  );

  console.log("🔘 Buttons on OTP screen:", buttons);
...
test("intro page - verify introduction anchor link exists", async ({
  page,
}) => {
  await page.goto("https://playwright.dev/docs/intro");
